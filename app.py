...
def _build_acumulado_acoes(registros, filtro_contratada=None, filtro_contrato=None):
    """Retorna {contratada||contrato: {acao: total_realizado}} somando todos os registros."""
    acum: dict = {}
    for r in registros:
        c, k = r.get('contratada', ''), r.get('contrato', '')
        if filtro_contratada and c != filtro_contratada:
            continue
        if filtro_contrato and k != filtro_contrato:
            continue
        key = contrato_key(c, k)
        for acao, val in (r.get('acoes_realizadas') or {}).items():
            try:
                acum.setdefault(key, {})[acao] = round(
                    acum.get(key, {}).get(acao, 0) + float(val or 0), 2)
            except Exception:
                pass
    return acum


def _recompute_forecast(cfg, contratada, contrato, registros):
    """Recalcula o forecast de um contrato do zero a partir do baseline + todos os registros.

    Para cada ação, parte do baseline e processa as semanas com realizado em ordem
    cronológica: a diferença (déficit ou superávit) entre o previsto e o realizado de
    cada semana é redistribuída igualmente entre as semanas futuras da mesma ação.

    Por recalcular sempre do zero, é idempotente — não acumula em edições e reflete
    corretamente exclusões de registros. Retorna True se o config foi alterado.
    """
    ...
    # Agrega realizados por ação e semana ISO somente deste contrato
    real_por_semana: dict = {}   # {acao: {semana_iso: realizado}}
    for r in registros:
        if r.get('contratada') != contratada or r.get('contrato') != contrato:
            continue
        sw = format_date_to_week(r.get('semana_referencia', ''))
        if not sw:
            continue
        for acao, val in (r.get('acoes_realizadas') or {}).items():
            try:
                real_por_semana.setdefault(acao, {})[sw] = round(float(val or 0), 2)
            except Exception:
                pass

    modificado = False
    for acao_entry in cdata.get('linha_base_acoes', []):
        acao_nome = (acao_entry.get('acao') or '').strip()
        semanas_base = acao_entry.get('semanas', {})

        # Forecast começa idêntico ao baseline
        forecast = {s: round(float(v or 0), 2) for s, v in semanas_base.items()}
        reais = real_por_semana.get(acao_nome, {})

        for sw in sorted(reais.keys()):
            if sw not in forecast:
                continue
            planejado = forecast.get(sw, 0)   # usa o forecast já revisado da semana
            if planejado == 0:
                continue
            realizado = reais[sw]
            ...
            delta = round(planejado - realizado, 2)   # >0 déficit, <0 superávit
            semanas_futuras = sorted(
                s for s, v in semanas_base.items()
                if s > sw and float(v or 0) > 0
            )
            if not semanas_futuras:
                continue

            n = len(semanas_futuras)
            parcela = round(delta / n, 2)
            resto = round(delta - parcela * n, 2)
            for idx, s in enumerate(semanas_futuras):
                ajuste = parcela + (resto if idx == n - 1 else 0)
                forecast[s] = round(max(0.0, forecast[s] + ajuste), 2)

        prev = acao_entry.get('forecast_semanal')
        if prev != forecast:
            acao_entry['forecast_semanal'] = forecast
            modificado = True
...
def novo():
    ...
    if request.method == 'POST':
        ...
        registros = load_data()
        for r in registros:
            if (r['contratada'] == contratada and r.get('contrato') == contrato
                    and r['semana_referencia'] == semana_referencia):
                erros.append(f'Já existe um registro para "{contratada} / {contrato}" na semana de {format_date_br(semana_referencia)}.')
                break
        ...
        # Recalcula o forecast de ações do contrato (déficit/superávit → semanas futuras)
        _cfg = load_contratos_config()
        if _recompute_forecast(_cfg, contratada, contrato, registros):
            save_contratos_config(_cfg)
    ...
    # Acumulado já entregue por ação (somando registros anteriores do mesmo contrato)
    _todos = load_data()
    _acumulado_acoes = _build_acumulado_acoes(_todos, _vc, _vk)
    ...
    return render_template('form.html',
                           ...
                           acumulado_acoes=_acumulado_acoes,
                           contratos_cfg_json=json.dumps(load_contratos_config()))
...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        for r in registros:
            if (r['id'] != id and r['contratada'] == contratada
                    and r.get('contrato') == contrato and r['semana_referencia'] == semana_referencia):
                erros.append(f'Já existe outro registro para "{contratada} / {contrato}" na semana de {format_date_br(semana_referencia)}.')
                break
    ...
    # Acumulado já entregue por ação (excluindo o próprio registro em edição)
    _outros = [r for r in load_data() if r.get('id') != id
               and r.get('contratada') == registro.get('contratada')
               and r.get('contrato')   == registro.get('contrato')]
    _acumulado_acoes = _build_acumulado_acoes(
        _outros,
        registro.get('contratada'), registro.get('contrato')
    )
...
def excluir(id):
    ...
    if alvo:
        # Recalcula o forecast do contrato afetado após remover o registro
        _cfg = load_contratos_config()
        if _recompute_forecast(_cfg, alvo.get('contratada', ''), alvo.get('contrato', ''), registros):
            save_contratos_config(_cfg)
...
def dashboard():
    ...
    filtro_contratada = request.args.get('contratada', '')
    filtro_contrato  = request.args.get('contrato', '')
    filtro_de        = request.args.get('de', '')
    filtro_ate       = request.args.get('ate', '')
    ...
    # Mapa contratada→contratos para filtro cascata no frontend (antes de filtrar por contratada)
    _mapa_contratos: dict = {}
    for _r in reg:
        _c, _k = _r.get('contratada', ''), _r.get('contrato', '')
        if _c and _k:
            _mapa_contratos.setdefault(_c, set()).add(_k)
    contratos_por_contratada_json = json.dumps({c: sorted(ks) for c, ks in _mapa_contratos.items()})

    if filtro_contratada:
        reg = [r for r in reg if r.get('contratada') == filtro_contratada]

    # Lista de contratos disponíveis para o render inicial (já filtrada por contratada)
    todos_contratos = sorted(set(r.get('contrato', '') for r in reg if r.get('contrato')))

    if filtro_contrato:
        reg = [r for r in reg if r.get('contrato') == filtro_contrato]
    if filtro_de:
        reg = [r for r in reg if r.get('semana_referencia', '') >= filtro_de]
    if filtro_ate:
        reg = [r for r in reg if r.get('semana_referencia', '') <= filtro_ate]
    ...
    _vazio = dict(kpis=None, contratadas=todas_contratadas,
                  ...
                  contratos_por_contratada_json=contratos_por_contratada_json,
                  filtro_contratada=filtro_contratada,
    ...
    curva_fin_base = fin_base_curve(cfg, filtro_contratada, semanas_sorted, filtro_contrato or None)
...
def visualizar(id):
    ...
    # Monta mapa {ação: planejado} para a semana do registro
    cfg = load_contratos_config()
    cdata = cfg.get(contrato_key(registro.get('contratada', ''), registro.get('contrato', '')), {})
    semana_iso = format_date_to_week(registro.get('semana_referencia', ''))
    acoes_planejado = {}
    acoes_unidade = {}
    for e in cdata.get('linha_base_acoes', []):
        nome = (e.get('acao') or '').strip()
        if not nome:
            continue
        acoes_unidade[nome] = (e.get('unidade') or '').strip()
        if semana_iso in e.get('semanas', {}):
            acoes_planejado[nome] = float(e['semanas'].get(semana_iso, 0) or 0)
    return render_template('visualizar.html', r=registro,
                           acoes_planejado=acoes_planejado, acoes_unidade=acoes_unidade)
...
def admin_contrato(key):
    ...
    # Monta mapa de realizados por ação e semana: {acao: {semana_iso: valor}}
    _c, _k = data.get('contratada', ''), data.get('contrato', '')
    acoes_real_map: dict = {}
    for r in load_data():
        if r.get('contratada') != _c or r.get('contrato') != _k:
            continue
        _sw = format_date_to_week(r.get('semana_referencia', ''))
        if not _sw:
            continue
        for acao, val in (r.get('acoes_realizadas') or {}).items():
            try:
                acoes_real_map.setdefault(acao, {})[_sw] = round(float(val or 0), 2)
            except Exception:
                pass

    return render_template('admin_contrato.html', key=key, contrato=data_view,
                           funcoes_mao_obra=get_funcoes_list(),
                           acoes_real_map=acoes_real_map)
